...
function ToolBar() {
  ...
  return (
    <div className={styles.toolbar}>
      <IconButton onClick={() => createUndoAction()}>
        <UndoIcon className={styles.button} sx={{ fontSize: 17 }} />
      </IconButton>
      <IconButton onClick={() => createRedoAction()}>
      ...
      {slides?.length == 1 ? (
        <IconButton disabled>
          <DeleteIcon className={styles.button} sx={{ fontSize: 17 }} />
        </IconButton>
      ) : (
        <IconButton onClick={() => removeSlide()}>
          <DeleteIcon className={styles.button} sx={{ fontSize: 17 }} />
        </IconButton>
      )}
      ...
      {selectedBlock?.type === "text" && (
        <div className={styles.flex}>
          ...
          <input
            ...
            type="number"
            value={(selectedBlock as TTextBlock).fontSize}
            onChange={(event) => changeFontSizeOfText(event.target.value)}
          />
        </div>
      )}
        ...
        <div className={styles.flex}>
          <IconButton>
            ...
            <input
              ...
              value={
                selectedBlock.type === "graphic"
                  ? (selectedBlock as TGraphicBlock).background
                  : (selectedBlock as TTextBlock).color
              }
      ...
      <IconButton onClick={() => addTextBlock()}>
        <TitleIcon className={styles.button} sx={{ fontSize: 17 }} />
      </IconButton>
      <IconButton>
        <label className={styles.button} htmlFor="image_uploads">
