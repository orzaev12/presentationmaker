  ...
  return (
    <div className="menu">
      <button className={styles.button}>
        <label className={styles.text} htmlFor="file_uploads">
          Загрузить
        </label>
      </button>
      <button
        className={styles.button}
        onClick={() => createSetSelectedBlockAction(currentSlide.id, null)}
      >
        <a className={styles.text} href={getURL()} download={title}>
          Сохранить JSON
        </a>
      </button>
      <button className={styles.button} onClick={() => getPdf()}>
        <a className={styles.text}>Сохранить PDF</a>
      </button>
      <input
