...
function TextBlock({ object, id }: TextBlockProps) {
  ...
  const { createAddCharacterAction, createDeleteCharacterAction } =
    useAppActions()
  ...
  return (
    <div ref={ref} className={styles.block}>
      <p className={styles.text} style={stylelist}>
        {object.value}
        {selectedBlockId === id && (
          <span className={styles.cursor} style={styleForCursor}></span>
        )}
      </p>
    </div>
  )
