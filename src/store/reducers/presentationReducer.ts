...
const presentationReducer = (state: Presentation = presentation, action: Action) => {
    switch (action.type) {
        ...
        case PresentationActions.ADD_SLIDE: {
            ...
            newSlides.splice(action.payload.indexOfCurrentSlide + 1, 0, {id: uuid(), background: '#FFFFFF', data: []})
            const newState = {
                ...state,
                slides: newSlides,
        ...
        case PresentationActions.REMOVE_SLIDE: {
            const newState = {
                ...state,
                slides: state.slides.filter(item => item.id != action.payload.slideId),
            }
            history.addHistoryItem(newState)
            return newState
        }
